...
def test_get_inventory_devices():
    ...
    """
    response = requests.get(f"{BASE_URL}/inventory/devices", verify=CA_BUNDLE)
    assert response.status_code == 200
    assert response.json() == default_inventory_devices_response['body']


def test_update_inventory_devices_with_valid_data():
    """
    ...
    response = requests.put(f"{BASE_URL}/inventory/devices", json=updated_data, verify=CA_BUNDLE)
    assert response.status_code == 200
    assert response.json()["new_body"] == updated_data["body"]
    assert response.json()["new_status_code"] == updated_data["status_code"]
    
def test_update_inventory_devices_with_invalid_data():
    """
    Test PUT /inventory/devices endpoint to update the devices response.
    """
    updated_data = {
...
def test_get_guids():
    ...
    """     
    response = requests.get(f"{BASE_URL}/guids", verify=CA_BUNDLE)
...
def test_post_guid_add():
    ...
    response = requests.post(f"{BASE_URL}/{guid}/add", verify=CA_BUNDLE)
    assert response.status_code == 200
    guid_add_response = {
        "body": {
            "guids": [guid]
    ...
    current_all_guids = requests.get(f"{BASE_URL}/guids", verify=CA_BUNDLE)
    assert current_all_guids.json() == guid_add_response["body"]
    assert current_all_guids.status_code == 200
    
def test_post_guid_add_duplicated_value():
    """
    Test POST /<guid>/add endpoint to add a duplicated GUID.
    """
    guid = "123e4567-e89b-12d3-a456-426614174000"
    get_all_guids = requests.get(f"{BASE_URL}/guids", verify=CA_BUNDLE)
    assert get_all_guids.json() == default_guids_response["body"]
    assert get_all_guids.status_code == 200
    requests.post(f"{BASE_URL}/{guid}/add", verify=CA_BUNDLE) 
    requests.post(f"{BASE_URL}/{guid}/add", verify=CA_BUNDLE)
    duplicated_response = {
        "body": {
            "guids": [guid, guid]
